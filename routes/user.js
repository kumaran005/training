...
/*exports.add_candidate = function (req, res) {
  ...
  if (req.method == "POST") {
    ...
    db.query(sql, function (err, data) {
      var sql =
        "INSERT INTO `ems`.`cand_profile_details`(`cand_id`,`name`,`initial`,`initial_expansion`,`father_name`,`mother_name`,`date_of_birth`,`gender`,`blood_group`,`religion`,`community`,`caste`,`nationality`,`willing_to_donate_blood`,`registered_time`,`last_modified_time`,`academic_year`,`student_code`) VALUES ('" +
        cand_id +
        "','" +
        name +
        "','" +
      ...
      query = db.query(sql, function (err, data) {
        var sql =
          "INSERT INTO `ems`.`cand_admission_details`(`cand_id`,`rank`,`rank_no`,`ar_no`,`total_mark`,`neet_mark`,`reg_no`,`neet_roll_no`,`course`,`admission_type`,`admission_quota`,`course_commencement`,`date_of_admission`,`date_of_allotment`,`selected_category`,`willing_for_counciling`,`last_modified_time`) VALUES ('" +
          cand_id +
          "','" +
          rank +
          "','" +
        ...
        query = db.query(sql, function (err, data) {
          var sql =
            "INSERT INTO `ems`.`cand_address_details`(`cand_id`,`address_type`,`ps_address`,`ps_pincode`,`ps_state`,`ps_district`,`pm_address`,`pm_pincode`,`pm_state`,`ps_district`,`last_modified_time`) VALUES ('" +
            cand_id +
            "','" +
            address_type +
            "','" +
          ...
          query = db.query(sql, function (err, data) {
            var sql =
              "INSERT INTO `ems`.`cand_contact_details`(`cand_id`,`tel_phone`,`mobile_phone`,`email_id`,`aadhar_no`,`voter_id`,`remarks`,`last_modified_time`) VALUES ('" +
              cand_id +
              "','" +
              tel_phone +
              "','" +
            ...
            query = db.query(sql, function (err, data) {
              var sql =
                "INSERT INTO `ems`.`audit_trail`(`event_name`,`current_user`,`audited_time`,`user_agent`) VALUES ('" +
                event_name +
                "','" +
                current_user +
                "','" +
...
exports.report1 = function (req, res, next) {
  ...
  var sql10 =
    "SELECT * FROM `ems`.`cand_certificate2_details` WHERE `cand_id`='" +
    cand_id +
    "'";
  db.query(sql10, function (err, data10) {
    var sql9 =
      "SELECT * FROM `ems`.`cand_certificate_details` WHERE `cand_id`='" +
      cand_id +
      "'";
    db.query(sql9, function (err, data9) {
      var sql8 =
        "SELECT * FROM `ems`.`cand_contact_details` WHERE `cand_id`='" +
        cand_id +
        "'";
      db.query(sql8, function (err, data8) {
        var sql7 =
          "SELECT * FROM `ems`.`cand_bank_details` WHERE `cand_id`='" +
          cand_id +
          "'";
        db.query(sql7, function (err, data7) {
          var sql6 =
            "SELECT * FROM `ems`.`cand_neet_mark_details` WHERE `cand_id`='" +
            cand_id +
            "'";
          db.query(sql6, function (err, data6) {
            var sql5 =
              "SELECT * FROM `ems`.`cand_marks_details` WHERE `cand_id`='" +
              cand_id +
              "'";
            db.query(sql5, function (err, data5) {
              var sql4 =
                "SELECT * FROM `ems`.`cand_institute_details` WHERE `cand_id`='" +
                cand_id +
                "'";
              db.query(sql4, function (err, data4) {
                var sql3 =
                  "SELECT * FROM `ems`.`cand_photo_details` WHERE `cand_id`='" +
                  cand_id +
                  "'";
                db.query(sql3, function (err, data3) {
                  var sql2 =
                    "SELECT * FROM `ems`.`cand_address_details` WHERE `cand_id`='" +
                    cand_id +
                    "'";
                  db.query(sql2, function (err, data2) {
                    var sql1 =
                      "SELECT * FROM `ems`.`cand_admission_details` WHERE `cand_id`='" +
                      cand_id +
                      "'";
                    db.query(sql1, function (err, data1) {
                      var sql =
                        "SELECT * FROM `ems`.`cand_profile_details` WHERE `cand_id`='" +
                        cand_id +
                        "'";
                      db.query(sql, function (err, data) {
...
exports.report2 = function (req, res, next) {
  ...
  var cand_id = post.cand_id;
  var sql11 =
    "SELECT * FROM `ems`.`cand_fees` WHERE `cand_id`='" + cand_id + "'";
  db.query(sql11, function (err, data11) {
    var sql10 =
      "SELECT * FROM `ems`.`cand_certificate2_details` WHERE `cand_id`='" +
      cand_id +
